...
def inexact_footprint_search(directory):
    ...
    with open(string + 'N1zones.csv', 'r') as sequences, \
        ...
        for seq in sequences:
            ...
            with open('addition_files/Footprints.txt', 'r') as footprints:
...
def vgen_footprints_dict():
    ...
    vgen_dict = {}
    with open('addition_files/V-gen-footprints.txt', 'r') as file_in:
        current_genes = []
        is_gen = True
        for line in file_in:
            t = line.split()
...
def find_genuine_footprints(directory, option):
    ...
    place = 0
    with open('addition_files/v-genes.csv', 'r') as v_genes:
        for line in v_genes:
            place += 1
            position_v_genes[line[: -1]] = place

    with open("addition_files/undefined_genes.txt", 'r') as undef_genes_file:
        undefined_genes = []
        ...
        for i, line in enumerate(f):
            ...
            if i % 9 == 7:
                ...
                if sequences_with_footprints.get(number) is None:
                    sequences_with_footprints[number] = [pos]
                else:
                    if pos not in sequences_with_footprints[number]:
                        sequences_with_footprints[number].append(pos)

    with open(string + option + '_result.txt', 'w+') as f:
        for seq in sequences_with_footprints.keys():
            f.write(seq + '   position fp:')
            pos_line = ' '
            for pos in sequences_with_footprints[seq]:
                pos_line += pos + ', '
            pos_line = pos_line[:-2]
            f.write(pos_line + '\n\n')

        f.write("Count of sequences with footprints: " + str(len(sequences_with_footprints.keys())))
...
if __name__ == "__main__":
    if len(argv) > 3:
        ...
        count_sequences(directory, option)
    else:
        print("Enter file with N1 zones, output directory and search type (exact\inexact)")
